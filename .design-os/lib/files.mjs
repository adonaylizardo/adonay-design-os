 ...
 * 2. A package.json named adonay-design-os
 * 
...
export function findWorkspaceRoot() {
  ...
  while (dir !== root && dir !== '/') {
    ...
    // Check for package.json for this workspace
    const pkgPath = join(dir, 'package.json');
    if (existsSync(pkgPath)) {
      try {
        const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'));
        if (
          pkg.name === 'adonay-design-os' ||
          pkg.dependencies?.['adonay-design-os'] ||
          pkg.devDependencies?.['adonay-design-os']
        ) {
          _workspaceRootCache = dir;
          return dir;
        }
