...
export async function run(args) {
  ...
  if (!useAgent) {
    ...
    if (slug && slug !== 'all') {
      console.log(`   Run: design review ${slug} --agent`);
    } else {
      console.log('   Run: design review <project> --agent');
    }
    console.log('');
  ...
  if (!slug || slug === 'all') {
    ...
    console.log('   Example: design review my-project --agent');
    console.log('');
    process.exit(1);
  }
  ...
  if (existsSync(designReviewPath)) {
    ...
    console.log(`   2. Run: design comment ${slug} --dry-run`);
    console.log('');
  } else {
